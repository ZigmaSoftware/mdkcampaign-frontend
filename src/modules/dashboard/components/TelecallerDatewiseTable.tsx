...
import type { TelecallerDatewisePerformanceRow } from '../services/dashboardApi'

interface TelecallerDatewiseTableProps {
...
export default function TelecallerDatewiseTable({ rows }: TelecallerDatewiseTableProps) {
  ...
  const uniqueDates = useMemo(
    () => new Set(rows.map(row => row.date).filter(Boolean)).size,
    [rows],
  )

  const uniqueTelecallers = useMemo(
    () => new Set(rows.map(row => `${row.telecaller_id ?? 'name'}:${row.telecaller_name}`)).size,
    [rows],
  )

  return (
    <Card
      title="Telecaller Performance By Date"
      icon="ph ph-calendar-blank"
      headerRight={
        <>
          <Badge label={`${uniqueDates} dates`} variant="s" />
          <Badge label={`${uniqueTelecallers} telecallers`} variant="blue" />
        </>
      }
      bodyClass="p-0"
      className="mb-0"
    >
      {rows.length === 0 ? (
        <div className="px-[18px] py-[18px] text-[11px] text-muted italic">
          No date-wise telecaller performance found for the current filter scope.
        </div>
      ) : (
        <>
          <div className="border-b border-border bg-[#f8fafc] px-[18px] py-[10px] text-[10px] text-muted">
            Daily breakdown using the same telecaller efficiency formula and current dashboard filters.
          </div>
          <div className="overflow-x-auto">
            <table className="data-table w-full text-[11px]">
              ...
              <tbody>
                {pagedRows.map(row => (
                  <tr key={`${row.date}-${row.telecaller_id ?? row.telecaller_name}-${row.rank}`}>
                    <td>
                      <div className="font-semibold text-navy">{formatDateLabel(row.date)}</div>
