...
function AppShell() {
  ...
  if (window.location.pathname === '/poll-system') {
    return <PublicPollPage />
  }

  // ── Obfuscated poll share link  e.g. /s/cG9sbC1zeXN0ZW0 ──
  if (window.location.pathname.startsWith('/s/')) {
    const token = window.location.pathname.slice(3)
    try {
      const decoded = atob(token.replace(/-/g, '+').replace(/_/g, '/'))
      if (decoded === 'poll-system') return <PublicPollPage />
  ...
  return (
    <div className="min-h-screen flex flex-col bg-bg font-inter text-textMain text-[14px]">
      ...
      <div className="relative">
        ...
        {/* Floating user pill */}
        <div className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 flex items-center gap-1.5 z-10">
          <span className="hidden sm:flex items-center gap-1.5 bg-white border border-border rounded-full px-3 py-1 text-[11px] font-semibold text-navy shadow-sm">
            <i className={`ph-fill ${currentUser.role === 'admin' ? 'ph-shield-star text-saffron' : 'ph-user-circle text-kampgreen'} text-[13px]`} />
            {currentUser.username}
            <span className={`ml-1 text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full ${currentUser.role === 'admin' ? 'bg-saffron/20 text-saffron-dark' : 'bg-kampgreen/10 text-kampgreen-dark'}`}>
              {currentUser.role}
            </span>
          </span>
          <button
