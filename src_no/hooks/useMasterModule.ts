...
interface UseMasterModuleReturn {
  ...
  addRecord:     (key: string, meta?: string) => void
  deleteRecord:  (id: string) => void
  updateRecord:  (id: string, key: string, meta?: string) => void
}
...
export function useMasterModule(module: MasterModuleId): UseMasterModuleReturn {
  ...
  const addRecord = useCallback((key: string, meta?: string) => {
    if (!key.trim()) {
  ...
  const updateRecord = useCallback((id: string, key: string, meta?: string) => {
    dispatch({ type: 'UPDATE_MASTER', module, id, key, meta })
